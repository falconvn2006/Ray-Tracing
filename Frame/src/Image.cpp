...
namespace Frame
{
	namespace Utils
	{
		...
		static VkFormat FrameFormatToVulkanFormat(ImageFormat format)
		{
			switch (format)
			{
	...
	void Image::AllocateMemory(uint64_t size)
	{
		...
		VkFormat vulkanFormat = Utils::FrameFormatToVulkanFormat(m_Format);
		...
		// Create the Image
		{
			...
			res = vkCreateImage(device, &info, nullptr, &m_Image);
			check_vk_result(res);
		...
		// Create the Image View:
		{
			...
			res = vkCreateImageView(device, &info, nullptr, &m_ImageView);
			check_vk_result(res);
		}
